...
var adapter = module.exports = (function () {
  ...
  function query(connection, collection, q, params, cb, unique) {
    connections[connection].graph(function (gr) {
      ...
      gr.run(q.join('\n'), params).then(function (results) {
        ...
        debugger;
        cb(null, _.map(results.records, function (record) {
          var _obj = record.toObject();
          _obj.n.properties.id = _obj.nodeId.low;
          return _obj.n.properties
        }));
  ...
  return {
    ...
    create: function (connection, collection, params, cb) {
      ...
      if (params !== null && collection !== null) {
        delimiter = ' AND ';
      } // do we have a label and params?
      ...
      query(connection, collection, q, params, function(error, records){
        if(error){
          return cb(error);
        }else{
          cb(null, records[0]);
    ...
    link: function (connection, collection, predecessorParams, successorCollectionName, successorParams, relationshipType, relationshipParams, cb) {
      debugger;
      var q, predecessorDelimiter = '',
        successorDelimiter = '',
        predecessorNamespace = 'pred',
        successorNamespace = 'succ';
      ...
      if (predecessorParams && collection) {
        predecessorDelimiter = ' AND ';
      }

      if (successorCollectionName === null) {
        successorCollectionName = '';
      } // do we have a label?
      else {
        successorCollectionName = 'b:' + successorCollectionName;
      }
      if (successorParams && collection) {
        successorDelimiter = ' AND ';
      }
      ...
      q = [
        'MATCH (a),(b)',
        'WHERE ' + collection + predecessorDelimiter + toWhere('a', {
          where: predecessorParams
        }, predecessorNamespace)
        +
        ' AND ' + successorCollectionName + successorDelimiter + toWhere('b', {
          where: successorParams
        }, successorNamespace),
